 ...
 * Slack MCP Server
 *
 * Community edition — connects directly to Slack Web API.
 *
 * Usage (stdio - for Claude Desktop / Cursor / VS Code):
 *   SLACK_BOT_TOKEN=xoxb-xxx npx @node2flow/slack-mcp
 *
...
async function startHttp() {
  ...
  app.post('/mcp', async (req: any, res: any) => {
    ...
    const qToken = url.searchParams.get('SLACK_BOT_TOKEN');
    if (qToken) process.env.SLACK_BOT_TOKEN = qToken;
  ...
  const config = getConfig();
  app.listen(port, () => {
    console.log(`Slack MCP Server (HTTP) listening on port ${port}`);
 ...
 */
export default function createSmitheryServer(opts?: { config?: { SLACK_BOT_TOKEN?: string } }) {
  if (opts?.config?.SLACK_BOT_TOKEN) process.env.SLACK_BOT_TOKEN = opts.config.SLACK_BOT_TOKEN;
  const config = getConfig();
